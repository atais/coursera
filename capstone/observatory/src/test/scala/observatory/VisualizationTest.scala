...
trait VisualizationTest extends FunSuite with Checkers with Matchers {
  ...
  val locateTemperatures = Extraction.locateTemperatures(year, stationsPath, temperaturePath)
  val locateAverage = Extraction.locationYearlyAverageRecords(locateTemperatures)
